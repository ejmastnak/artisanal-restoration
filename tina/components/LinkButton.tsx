...
export default function LinkButton({ tinaField, className, children, ...props }: Props) {
  ...
  return (
    <a
      ...
      className={`inline-flex gap-x-1 items-center text-center rounded-full bg-theme-tan px-4 py-2.5 text-sm font-semibold text-white shadow-sm border border-theme-darktan hover:bg-theme-darktan focus-visible:outline focus-visible:outline-2 focus-visible:border-theme-tan focus-visible:outline-offset-2 focus-visible:outline-theme-darktan ${className ?? ""}`}
      {...props}
    >
      {children}
