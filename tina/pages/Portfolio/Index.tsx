  ...
  return (
    <PageWrapper>
      ...
      <div className="mt-12 flex flex-col gap-y-24 md:gap-y-28 max-w-5xl mx-auto">
        {portfolioPage.featuredProjects.filter((project) => !!(project.project)).map((project, idx) => (
          <div key={project.project.id} data-tina-field={tinaField(project, "project")} className={`md:flex gap-x-10 ${idx %2 == 1 ? 'md:flex-row-reverse' : ''}`}>
            <div className="text-gray-800 max-w-xl mx-auto">
              ...
              {/* Mobile image */}
              <a href={`/portfolio/${project.project._sys.filename}`} className="w-full h-fit">
                <img
              ...
              {(project.project.writeUpSections || project.project.gallerySections) && 
                <p className="mt-2 font-semibold text-gray-700">
                  ...
                  {(project.project.gallerySections && !project.project.writeUpSections) && 
              ...
              <div className="mt-8 w-fit block mx-auto">
                <LinkButton data-tina-field={tinaField(portfolioPage, "linkButtonText")} href={`/portfolio/${project.project._sys.filename}`}>
                  {portfolioPage.linkButtonText}
                </LinkButton>
              </div>
            </div>
            <a href={`/portfolio/${project.project._sys.filename}`} className="w-full max-w-sm h-fit">
              <img
                src={project.project.featuredImage}
                alt={project.project.featuredImageAlt}
